...
<body>
    <div class="document">
<h1>!/bin/bash</h1>
<h1>Auto-generated regeneration script for job application</h1>
<h1>Created: 2025-06-25T02:00:21.042481</h1>
<h1>AI Provider: Unknown (Unknown)</h1>
...
<h1>Check if we're in the right directory</h1>
<p>if [ ! -f "Makefile" ] || [ ! -d "src" ]; then<br />
    echo "${RED}❌ Error: Not in project root directory${NC}"<br />
    echo "Please run this script from the Bewerbung project root"<br />
    exit 1<br />
fi</p>
<h1>Check dependencies</h1>
<p>echo "🔍 Checking dependencies..."</p>
<h1>Check Python</h1>
<p>if ! command -v python3 &amp;&gt; /dev/null; then<br />
    echo "${RED}❌ Python3 not found${NC}"<br />
    exit 1<br />
fi</p>
<h1>Check virtual environment</h1>
<p>if [ ! -d ".venv" ]; then<br />
    echo "${YELLOW}⚠️  Virtual environment not found, creating one...${NC}"<br />
...
<h1>Set environment variables for exact reproduction</h1>
<p>export AI_PROVIDER="claude"<br />
export OUTPUT_STRUCTURE="by_model"<br />
export INCLUDE_GENERATION_METADATA="true"</p>
<h1>Check AI provider availability</h1>
<p>echo "🤖 Checking AI provider availability..."<br />
make test-providers || echo "${YELLOW}⚠️  Some AI providers may not be available${NC}"</p>
<h1>Run generation</h1>
<p>echo "🚀 Starting generation..."<br />
make generate</p>
<p>echo "${GREEN}✅ Regeneration completed successfully!${NC}"<br />
